...
The script only reads pre-generated reports — it never runs eslint/jscpd/jest itself. Generate them first, in the target project's root:

```bash
npx eslint . --format json > eslint-report.json
npx jscpd . --reporters json          # writes jscpd-report.json
npx jest --coverage --coverageReporters=json-summary   # writes coverage/coverage-summary.json
```
...
- **"required file not found: eslint-report.json"** → run the collection command for that tool first; the script never runs it for you.
