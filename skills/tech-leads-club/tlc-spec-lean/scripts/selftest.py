...
CASES = [
    (
        "validate_checks",
        "checks.md",
        "drop one member from a size-5 set",
        lambda s: s.replace(" · `trial_will_end` C10 | - |", " | - |", 1),
        1,
        "declares 5 members but only 4",
    ),
    (
        "validate_checks",
        "checks.md",
        "remove a check's Proof line",
        lambda s: re.sub(
            r"\nProof: `bin/rails test test/billing/access_test\.rb[^\n]*\n", "\n", s
        ),
        1,
        "C4 has no `Proof:` line",
    ),
    (
        "validate_checks",
        "checks.md",
        "delete the concurrency sweep line",
        lambda s: s.replace("- concurrency: C7\n", ""),
        1,
        "dimension 'concurrency' has no line",
    ),
    (
        "validate_checks",
        "checks.md",
    ...
    (
        "validate_checks",
        "checks.md",
        "reference an undefined check",
        lambda s: s.replace("into it C1 ·", "into it C99 ·"),
        1,
        "C99 is referenced in Coverage",
    ),
    (
        "validate_checks",
        "checks.md",
        "duplicate check id",
        lambda s: s.replace(
            "**C6** - A `Suspended` subscription returns",
            "**C5** - A `Suspended` subscription returns",
        ),
        1,
        "duplicate check id C5",
    ),
    (
        "validate_checks",
        "checks.md",
        "non-empty Unproven cell",
        lambda s: s.replace(
            "| C2, table-driven over all 9 | - |",
            "| C2, table-driven over all 9 | `paused` |",
        ),
        1,
        "Unproven is '`paused`'",
    ),
    ...
    (
        ...
        "vague claim (warn)",
        lambda s: s.replace(
            "denies access to paid access groups", "handles access gracefully"
        ),
        0,
    ...
    (
        "validate_plan",
        "plan.md",
        "missing Observable section",
        lambda s: s.replace("## Observable", "## User-facing notes"),
        1,
        "missing required section: ## Observable",
    ),
    (
        "validate_plan",
        "plan.md",
        "empty Observable section",
        lambda s: re.sub(
            r"## Observable\n.*?\n## Flow", "## Observable\n\n## Flow", s, flags=re.S
        ),
        1,
        "Observable section is empty",
    ),
    (
        "validate_plan",
        "plan.md",
        "an Observable row with a blank landing",
        lambda s: s.replace(
            "| error shape and codes | AC 6 |", "| error shape and codes |  |"
        ),
        1,
        "landing is blank",
    ),
    (
        "validate_plan",
        "plan.md",
        "an Observable n/a with no reason",
        lambda s: s.replace(
            "| versioning | n/a - the provider pins the payload version in the envelope |",
            "| versioning | n/a |",
        ),
        1,
        "`n/a` with no reason",
    ...
    (
        ...
        1,
        "missing required section: ## Relations",
    ),
    (
        "validate_plan",
        "plan.md",
        "empty Surface section",
        lambda s: re.sub(
            r"## Surface\n.*?\n## Landing", "## Surface\n\n## Landing", s, flags=re.S
        ),
        1,
        "Surface section is empty",
    ),
    (
        "validate_plan",
        "plan.md",
        ...
        1,
        "Landing section is empty",
    ),
    (
        "validate_plan",
        "plan.md",
        "Landing door with no rejected alternative",
        lambda s: s.replace(
            "| a boolean `is_suspended` - cannot express the next state |", "|  |"
        ),
        1,
    ...
    (
        "validate_plan",
        "plan.md",
        "Landing door with no literal shape",
        lambda s: s.replace(
            "| enum value, not null, existing rows backfilled to `active` |", "|  |"
        ),
        1,
        "no literal shape",
    ),
    (
        "validate_plan",
        "plan.md",
        "columns and types creep into Relations",
        lambda s: s.replace(
        ...
        1,
        "carries an attribute block",
    ),
    (
        "validate_plan",
        "plan.md",
        "a Surface route with no statuses",
        lambda s: s.replace("| `200`, `409`, `422` |", "|  |"),
        1,
        "names no status code",
    ),
    (
        "validate_plan",
        "plan.md",
        "check ids written into Surface before checks exist",
        lambda s: s.replace(
            "| `200`, `409`, `422` |", "| `200`, `409`, `422` | C7, C8, C9 |"
        ),
        1,
        "does not exist yet",
    ),
    (
        ...
        1,
        "Impact section is empty",
    ),
    (
        "validate_plan",
        "plan.md",
        "a Flow hop naming a module that neither exists nor is a door (warn)",
        lambda s: s.replace(
        ...
        0,
        "without marking it as existing or as a",
    ),
    (
        "validate_plan",
        "plan.md",
        "a source marked binding under profile standard (warn)",
        lambda s: s.replace(
            "- provider webhook reference - the 9 statuses",
            "- design `03` - **binding for the interface** - the 9 statuses",
        ),
        0,
        "declares profile standard, so nobody",
    ),
    (
        "validate_checks",
        "plan.md",
        "a route reviewed in the plan that no check mentions (warn)",
        lambda s: s.replace(
            "`POST /webhooks/provider`", "`POST /webhooks/provider/v2`"
        ),
        0,
        "owe a Coverage row, or the route is dead",
    ),
    (
        "validate_plan",
        "plan.md",
        "acceptance criterion with no SHALL",
        lambda s: s.replace(
            "THEN the system SHALL set the subscription status",
            "THEN we set the subscription status",
        ),
        1,
        "has no SHALL",
    ),
    (
        "validate_plan",
        "plan.md",
        "assumption with no rationale",
        lambda s: s.replace(
            "| the provider already retried 3 times before it reports a failure |",
            "|  |",
        ),
        1,
        "empty 'Rationale'",
    ...
    (
        "validate_plan",
        "plan.md",
        "out hop naming a slug in backticks is not an unresolved module",
        lambda s: s.replace(
    ...
    (
        "validate_plan",
        "plan.md",
        "flowchart node neither existing nor a door (warn)",
        lambda s: s.replace(
            "3. `Billing::StatusMap` (new, no door - placement per conventions) - provider status -> local status",
    ...
    (
        "validate_plan",
        "plan.md",
        "open question left unresolved (warn)",
        lambda s: s.replace(
            "**Open questions:** none - all resolved or logged above.",
            "**Open questions:** what happens on a chargeback?",
        ),
        0,
        "do not read as resolved",
    ),
    (
        "validate_verification",
        "verification.md",
        "surviving mutant next to PASS",
        lambda s: s.replace(
            "| `app/webhooks/ingest.rb:31` | yes |",
            "| `app/webhooks/ingest.rb:31` | no |",
        ),
        1,
        "a mutant survived",
    ),
    (
        "validate_verification",
        "verification.md",
        "PASS with an unproven coverage member",
        lambda s: s.replace(
            "`trial_will_end` C10 | - |", "`trial_will_end` C10 | `paused` |"
        ),
        1,
        "unproven",
    ),
    (
        "validate_verification",
        "verification.md",
        "profile standard with no fault rows",
    ...
    (
        "validate_verification",
        "verification.md",
        "PASS with no file:line evidence",
        lambda s: re.sub(r"`[\w./-]+\.rb:\d+`", "`the test file`", s),
        1,
        "cites no file:line evidence",
    ),
    (
        "validate_verification",
        "verification.md",
    ...
    (
        "validate_verification",
        "verification.md",
        "a check row that is not PASS",
        lambda s: s.replace(
            "`assert_equal 1, WebhookDelivery.count` | PASS |",
    ...
    (
        "validate_verification",
        "verification.md",
        "self-verified report (warn)",
        lambda s: s.replace(
            "independent sub-agent (author != verifier)",
            "self-verified (degraded - no sub-agent)",
        ),
        0,
        "self-verified",
    ),
    (
        "validate_verification",
        "verification.md",
        "report downgrades the approved profile",
        lambda s: s.replace("**Profile**: standard", "**Profile**: light"),
        1,
        "checks.md was approved under 'standard'",
    ),
    # Profile-scoped steps: raising the profile in checks.md must make the report owe a section.
    (
        "validate_verification",
        "checks.md",
        "profile ui but no binding-sources section in the report",
        lambda s: s.replace("Profile: standard", "Profile: ui"),
        1,
        "no `## Binding sources` section",
    ),
    (
        "validate_verification",
        "verification.md",
        "profile standard with no recomputed Coverage",
        lambda s: re.sub(
            r"## Coverage\n.*?\n## Test policy rows",
            "## Test policy rows",
            s,
    ...
    (
        "validate_verification",
        "verification.md",
        "an unmet Test policy row next to PASS",
        lambda s: s.replace("| own layer C2 | yes |", "| own layer C2 | no |"),
        1,
        "Test policy row is unmet",
    ),
]
...
NEGATIVE_CONTROLS = [
    (
        "validate_verification",
        "a light report with no faults and no coverage is accepted",
        {
            "checks.md": lambda s: s.replace("Profile: standard", "Profile: light"),
            "verification.md": lambda s: re.sub(
                r"## Coverage\n.*?\n## Faults injected\n.*?\n## Gate",
                "## Gate",
                s.replace("**Profile**: standard", "**Profile**: light"),
                flags=re.S,
            ),
        },
    ),
...
def run(validator, root):
    p = subprocess.run(
        [
            sys.executable,
            os.path.join(HERE, f"{validator}.py"),
            FEATURE,
            "--root",
            root,
        ],
...
TOOLING = [
    (
        ...
        ["feat: suspend the subscription on a failed charge"],
        0,
        "OK",
    ),
    (
        "check_commit.py",
        "a non-conventional message fails",
        ["Updated stuff."],
        1,
        "does not match",
    ),
    (
        "check_commit.py",
        "a capitalized description fails",
        ["feat: Suspend the subscription"],
        1,
        "start lowercase",
    ),
    (
        "check_commit.py",
        "an unknown type fails",
        ["wip: suspend the subscription"],
        1,
        "is not one of",
    ),
    ...
    (
        "lessons.py",
        "--root before the subcommand is the documented form",
        ["--root", tempfile.gettempdir(), "list", "--status", "confirmed"],
        0,
        "",
    ),
]
...
def run_tooling():
    ...
    for script, desc, argv, want_code, want_frag in TOOLING:
        ...
        ok = False
        print(
            f"  BROKEN  {script}: {desc} - expected exit={want_code} and {want_frag!r}, "
            f"got exit={code}"
        )
    ...
    try:
        for argv, want, desc in [
            ...
            (
                ["--root", empty, "--allow-empty"],
                0,
                "an explicit empty sweep is allowed",
            ),
            (
                ["missing-feature", "--root", empty],
                2,
                "an unresolvable feature exits 2",
            ),
        ]:
            ...
            if code == want:
                ...
                ok = False
                print(
                    f"  BROKEN  validate_verification.py: {desc} - expected exit={want}, "
    ...
    try:
        steps = [
            ...
            (
                [
                    ...
                    "app/billing/x.rb:31",
                    "--text",
                    "assert the dedup path returns the row",
                ],
                "ADDED",
            ),
        ...
        for argv, frag in steps:
            ...
            if code != 0 or frag.lower() not in out.lower():
                ok = False
                print(
                    f"  BROKEN  lessons.py: round-trip step {argv[2]!r} - expected {frag!r}, "
            ...
            for artifact in ("lessons.json", "LESSONS.md"):
                if not os.path.isfile(os.path.join(root, ".specs", artifact)):
                    ok = False
                    print(
                        f"  BROKEN  lessons.py: round-trip wrote no .specs/{artifact}"
                    )
                    break
            else:
                print(
                    "  ok      lessons.py: init - add - status round-trip writes both artifacts"
                )
    finally:
...
def main(argv=None):
    ...
    for validator, artifact, desc, mutate, want_code, want_msg in CASES:
        ...
        if mutated == original:
            survived += 1
            print(
                f"  VACUOUS {validator}: {desc} - the mutation did not apply, so the case proves nothing"
            )
            shutil.rmtree(root)
            continue
        ...
        if code == want_code and want_msg in out:
            ...
            print(
                f"           expected exit={want_code} and {want_msg!r}, got exit={code}"
            )
            print("           " + out.strip().replace("\n", "\n           "))
        shutil.rmtree(root)
    ...
    for validator, desc, edits in NEGATIVE_CONTROLS:
        ...
        for artifact, edit in edits.items():
            ...
            if mutated == original:
                controls_ok = False
                print(
                    f"  VACUOUS {validator}: {desc} - the edit to {artifact} did not apply"
                )
            with open(path, "w", encoding="utf-8") as f:
                f.write(mutated)
        code, out = run(validator, root)
