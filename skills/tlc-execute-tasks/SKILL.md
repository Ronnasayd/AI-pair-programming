...
| 3.5 | Build wave groups        | Group tasks by `metadata.wave` (no wave metadata → wave 0 / sequential). PARALLEL = 2+ tasks in wave; SEQUENTIAL = 1 task or no wave metadata                                                    | Show wave breakdown, e.g. `Wave 0 (PARALLEL): T0, T1 — 2 tasks`                                                                                                              |
| 4   | Execute by wave          | Dispatch subagents per wave (see `references/orchestrator-model.md` for dispatch rules, `references/agent-prompts.md` for the exact prompt template); wait for all of wave N before starting N+1 | Per-task status: Complete/Blocked/Partial + files changed + gate result                                                                                                      |
