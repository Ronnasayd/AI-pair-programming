...
| 4   | Execute by wave          | PARALLEL (2+ tasks in wave): dispatch subagents (see `references/orchestrator-model.md`, `references/agent-prompts.md`). SEQUENTIAL (1 task, no wave, or all sequential): main agent executes inline, no subagent. Wait for all of wave N before starting N+1 | Per-task status: Complete/Blocked/Partial + files changed + gate result                                                                                                      |
