...
| 4   | Execute waves               | see below; use `TaskCreate`/`TaskGet`/`TaskList`/`TaskUpdate` to track wave/task progress alongside taskmaster MCP updates | each task → done/cancelled/deferred/blocked; worktrees branch off run branch, task diffs applied+committed onto run branch                                                                            |
...
- **PARALLEL** wave: batch ≤3 tasks concurrently. For each task, build context package (Step 4a) → pick executor model (Step 4a.5) → spawn executor (isolated worktree, branched off the run branch created in Step 0) + evaluator via `Agent` (templates: `references/agent-prompts.md`) → drive loop (exec→eval→log→stagnation-check→decide, cap 8 iter, per `adversarial-dev` Steps 2-4). Wait for whole batch before next batch; whole wave before next wave.
...
**4a context package**: task object, scoped spec/design excerpt, file paths (spec/design/metadata), scoped test/lint cmd (narrower than module-wide), expected file paths if known. Executor may commit freely inside its own worktree (branched off the run branch) — commits never leave that worktree/branch until merged in 4c. No commits to the original head branch until the user does the final commit after Step 6's review merge.
...
**4c merge**: for each APPROVED task, apply its worktree diff (`git apply`, staged/unstaged) onto the run branch's working tree, sequentially in task order. Conflict → `resolve-merge-conflicts` skill.
