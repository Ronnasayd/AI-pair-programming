Checklist for `sd-execute` skill, sequential:

1. **Validate tag** — check `.specs/features/{TAG}/spec.md` exists + matching tag in `.taskmaster/tasks/tasks.json`. Missing spec → stop, report. Missing taskmaster tag → tell user run `sd-insert-taskmaster` first, or proceed w/o sync.
