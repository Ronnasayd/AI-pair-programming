...
After the files exist, ask the user if they want to register the feature in taskmaster. If they answer `yes`, use the skill **sd-insert-taskmaster** -- it derives tasks from `tasks.md` when present, or falls back to one task per requirement/AC from `spec.md` when Tasks was skipped.
