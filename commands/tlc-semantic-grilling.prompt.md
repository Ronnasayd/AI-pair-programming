Run a `/grilling` session with `/tlc-spec-driven` in specify mode, but first, use tools like `rag-rat`,`serena` o any available tool to perform a semantic search to gather all the necessary information.
