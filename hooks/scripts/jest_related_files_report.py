...
_IMPORT_RE = re.compile(
    r"""(?:from\s+|require\(\s*|import\(\s*)['"](\.{1,2}/[^'"]+)['"]"""
)
MAX_RESULTS = 10
RAG_RAT_TIMEOUT = 15
...
def _symbol_stem(path: Path) -> str:
    """Base name for a rag-rat symbol query, stripping a trailing .test/.spec
    too (Path.stem only strips one extension, leaving e.g. "Tenant.spec")."""
    return _TEST_RE.sub("", path.name) or path.stem


def _resolve_relative_import(source_file: Path, import_spec: str) -> Path | None:
    candidate = (source_file.parent / import_spec).resolve()
    if candidate.suffix:
...
def _grep_imports_in_file(path: Path) -> list[Path]:
    try:
    ...
    for m in _IMPORT_RE.finditer(text):
        f = _resolve_relative_import(path, m.group(1))
...
def _grep_tests_importing(target: Path, project_root: str) -> list[Path]:
    ...
    for rel in result.stdout.splitlines():
        ...
        if not _is_test_file(test_path):
            continue
        imports = _grep_imports_in_file(test_path)
        if any(f == target for f in imports):
            matches.append(test_path)
    return matches


def _fallback_for_test_file(resolved: Path) -> list[str]:
    imports = _grep_imports_in_file(resolved)
...
def _rag_rat_for_test_file(resolved: Path, project_root: str) -> list[str] | None:
    text = call_rag_rat_tool(
        "impact_surface",
        {"query": _symbol_stem(resolved)},
        project_root,
        logger,
    ...
    if not text:
        return None
    paths = _rag_rat_paths_from_impact(text, "callees")
    filtered = [
        p for p in paths if Path(p).suffix in _JS_TS_EXTS and not _is_test_file(Path(p))
    ]
...
def build_related_files_context(file_path: str | None) -> str | None:
    ...
    if is_rag_rat_available(project_root):
        related = (
            _rag_rat_for_test_file(resolved, project_root)
            if is_test
            else _rag_rat_for_source_file(resolved, project_root)
        )
    ...
    if related is None:
        related = (
            _fallback_for_test_file(resolved)
            if is_test
