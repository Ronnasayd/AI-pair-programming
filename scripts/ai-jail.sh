...
PASSTHROUGH_VARS=(CLAUDE_PROJECT_DIR AI_PROJECT_ROOT_DIR CLAUDE_PROJECT_DIR ANTHROPIC_BASE_URL ANTHROPIC_MODEL ANTHROPIC_API_KEY ENABLE_TOOL_SEARCH ASDF_NODEJS_VERSION CONTEXT7_API_KEY)
...
for v in "${PASSTHROUGH_VARS[@]}"; do
    [ -n "${!v:-}" ] && PASSTHROUGH_ENV+=("--setenv" "$v" "${!v}")
done
