...
APP_NAME="rag-rat"
APP_VERSION="0.23.2"
if [ -n "${RAG_RAT_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$RAG_RAT_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${RAG_RAT_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$RAG_RAT_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/cq27-dev/rag-rat/releases/download/v0.23.2"
elif [ -n "${RAG_RAT_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$RAG_RAT_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/cq27-dev/rag-rat/releases/download/v0.23.2"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/cq27-dev/rag-rat/releases/download/v0.23.2"
fi
if [ -n "${RAG_RAT_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$RAG_RAT_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${RAG_RAT_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$RAG_RAT_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${RAG_RAT_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$RAG_RAT_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${RAG_RAT_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${RAG_RAT_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${RAG_RAT_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"rag-rat","name":"rag-rat","owner":"cq27-dev","release_type":"github"},"version":"0.23.2"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/rag-rat"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/rag-rat"
fi

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
rag-rat-installer.sh

The installer for rag-rat 0.23.2

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/cq27-dev/rag-rat/releases/download/v0.23.2
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set RAG_RAT_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in
        "rag-rat-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="08259a8dbb23afef1ce8134b7da0021ff3a4e10848a1a55a985b3c6e197912ea"
            _bins="rag-rat"
            _bins_js_array='"rag-rat"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            ...
            _updater_bin=""
            ;;
        "rag-rat-aarch64-unknown-linux-gnu.tar.xz")
            _arch="aarch64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="ffc903a1969c0f861c841879c555594202a133e2105da505ee5acb93a8c04d4e"
            _bins="rag-rat"
            _bins_js_array='"rag-rat"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "rag-rat-x86_64-pc-windows-msvc.zip")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".zip"
            _checksum_style="sha256"
            _checksum_value="55c9a640afce26bf743484311b375f363429d71f124d6bc664c78ffbfbe633d2"
            _bins="rag-rat.exe"
            _bins_js_array='"rag-rat.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            ...
            _updater_bin=""
            ;;
        "rag-rat-x86_64-unknown-linux-gnu.tar.xz")
            _arch="x86_64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="b917475b3d3aeb6b959fb372a904835d9d2822c3f7b880ce1bf9cc646567dcbe"
            _bins="rag-rat"
            _bins_js_array='"rag-rat"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in
        "aarch64-apple-darwin")
            _archive="rag-rat-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="rag-rat-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="rag-rat-aarch64-unknown-linux-gnu.tar.xz"
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="rag-rat-x86_64-unknown-linux-gnu.tar.xz"
            if ! check_glibc "2" "39"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
...
install() {
    ...
    if [ -n "${RAG_RAT_INSTALL_DIR:-}" ]; then
        _force_install_dir="$RAG_RAT_INSTALL_DIR"
