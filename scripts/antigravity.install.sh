...
########################################################################################
mkdir -p $LOCAL/$DEFAULT_FOLDER/rules/
find "$LOCAL/$DEFAULT_FOLDER/rules" -maxdepth 1 -type l | while read -r link; do
    target=$(readlink "$link")
    if [[ "$target" == "$SOURCE/instructions/"* ]]; then
        rm "$link"
    fi
done
ln -s "$SOURCE/instructions/"* "$LOCAL/$DEFAULT_FOLDER/rules/"
##########################################################################################
...
git_exclude \
    "GEMINI.md" \
    "$DEFAULT_FOLDER/rules/*" \
    "$DEFAULT_FOLDER/skills/*" \
    "$DEFAULT_FOLDER/mcp_config.json"
###########################################################################################
